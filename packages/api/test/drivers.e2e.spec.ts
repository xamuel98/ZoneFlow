...
async function createDriverUser(app: any, email?: string) {
  ...
  describe('Public Invitation Endpoints', () => {
    ...
    describe('POST /api/v1/drivers/accept-invitation', () => {
      it('should accept valid invitation and create driver account', async () => {
        ...
        // Create an invitation with unique email
        const uniqueEmail = `accept.test.${Date.now()}@example.com`;
        const createInvite = await app.request('/api/v1/drivers/invitations', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${ownerToken}` },
          body: JSON.stringify({
  ...
  describe('Driver Management Endpoints', () => {
    ...
    describe('POST /api/v1/drivers', () => {
      it('should create a new driver successfully', async () => {
        ...
        const json = await response.json();
        expect(json.success).toBe(true);
        expect(json.data.driver.name).toBe(driverData.name);
        expect(json.data.driver.email).toBe(driverData.email);
        expect(json.data.driver.phone).toBe(driverData.phone);
        expect(json.data.driver.vehicle_type).toBe(driverData.vehicleType);
        expect(json.data.driver.license_plate).toBe(driverData.licensePlate);
        expect(json.data.driver.is_available).toBe(1);
      }, 15000);
  ...
  describe('Bulk Operations', () => {
    describe('POST /api/v1/drivers/bulk', () => {
      it('should create multiple drivers successfully', async () => {
        ...
        expect(json.data.successfulDrivers).toHaveLength(2);
      }, 15000);
